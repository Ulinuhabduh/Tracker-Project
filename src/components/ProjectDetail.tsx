  ...
  return (
    <div className="space-y-8 animate-fade-in pb-16">
      ...
      <div className="flex flex-wrap items-center justify-between gap-4">
        <button
          onClick={onBack}
          className="flex items-center gap-2 px-3 py-1.5 rounded-xl border border-zinc-800 bg-zinc-900/60 hover:bg-zinc-800 text-xs font-semibold text-zinc-300 hover:text-white transition-all"
        >
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl border border-zinc-800 bg-zinc-900/60 hover:bg-zinc-800 text-xs font-semibold text-zinc-300 hover:text-white transition-all"
          >
      ...
      {/* Project Hero Banner */}
      <div className="glass-card rounded-3xl p-6 sm:p-8 border border-zinc-800/80 relative overflow-hidden">
        ...
        <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6 relative z-10">
          <div className="space-y-3 max-w-3xl">
            <div className="flex flex-wrap items-center gap-2.5">
              <span className="text-xs font-mono font-semibold px-2.5 py-1 rounded-lg bg-zinc-800 text-zinc-200 border border-zinc-700">
                {projectData.category}
              </span>
              <span className={`text-xs font-medium px-2.5 py-1 rounded-lg border ${priorityMeta.bg} ${priorityMeta.text} ${priorityMeta.border}`}>
                Prioritas {priorityMeta.label}
              </span>
              <span className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-lg text-xs font-medium border ${statusMeta.bg} ${statusMeta.text} ${statusMeta.border}`}>
          ...
          <div className="flex flex-col sm:flex-row items-center gap-5 p-5 rounded-2xl bg-zinc-950/60 border border-zinc-800 self-start lg:self-center shrink-0">
            <div className="text-center sm:text-left">
              ...
              <div className="w-32 bg-zinc-800 rounded-full h-2 mt-2 overflow-hidden">
                <div
            ...
            <div className="border-t sm:border-t-0 sm:border-l border-zinc-800 pt-3 sm:pt-0 sm:pl-5 flex flex-col gap-1 text-xs text-zinc-400">
              <div><strong>{projectData.tasks.length}</strong> Total Tasks</div>
              <div><strong>{projectData.milestones.length}</strong> Milestones</div>
              <div><strong>{projectData.logbooks.length}</strong> Logbook Entries</div>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="flex items-center gap-2 border-b border-zinc-800 pb-2">
        ...
        >
          ...
          <span>Jurnal & Logbook (Live Preview)</span>
          <span className="px-1.5 py-0.2 rounded-full text-[11px] bg-zinc-800 font-mono text-zinc-300">
      ...
      {activeTab === 'overview' && (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div className="glass-card rounded-2xl p-6 border border-zinc-800 space-y-4">
            <h3 className="text-base font-bold text-white tracking-tight">Teknologi & Tags Proyek</h3>
            {projectData.tags && projectData.tags.length > 0 ? (
              <div className="flex flex-wrap gap-2">
                {projectData.tags.map((tag, idx) => (
                  <span
                    key={idx}
                    className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-xs font-mono font-medium bg-zinc-800/90 text-zinc-200 border border-zinc-700"
                  >
            ...
            <div className="pt-4 border-t border-zinc-800 text-xs space-y-2 text-zinc-400">
              ...
              <div className="flex justify-between">
                ...
                <span className="font-mono text-[11px] text-zinc-500">{projectData.id}</span>
              </div>
            </div>
          </div>
          ...
          <div className="glass-card rounded-2xl p-6 border border-zinc-800 space-y-4">
            <h3 className="text-base font-bold text-white tracking-tight">Status Kesehatan Proyek</h3>
