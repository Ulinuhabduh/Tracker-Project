...
export function TaskManager({
  ...
  const handleToggleTaskStatus = async (task: Task) => {
    ...
    // Check if this action makes the project 100% complete
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
        ...
        <div className="glass-card rounded-2xl p-5 border border-zinc-800">
          ...
          <div className="mt-4 flex items-center justify-between text-xs text-zinc-400">
            <span>{completedCount} dari {totalCount} task terselesaikan</span>
        ...
        {/* Milestones Card */}
        <div className="lg:col-span-2 glass-card rounded-2xl p-5 border border-zinc-800">
          ...
          {/* New Milestone Form */}
          {isAddingMs && (
            <form onSubmit={handleCreateMilestone} className="mb-4 p-3 rounded-xl bg-zinc-900 border border-zinc-800 animate-fade-in space-y-2">
              <input
                type="text"
                required
                placeholder="Nama milestone penting..."
                value={msTitle}
            ...
            {milestones.length === 0 ? (
              ...
              milestones.map((ms) => (
                ...
                >
                  ...
                  <span className={ms.is_completed ? 'line-through opacity-80 font-medium' : 'font-medium'}>
                    {ms.title}
                  </span>
                  {ms.due_date && (
      ...
      <div className="space-y-4">
        ...
        {isAddingTask && (
          ...
          >
            ...
            <input
              type="text"
              required
              placeholder="Apa yang perlu diselesaikan? (mis: Integrasi autentikasi Supabase)"
              value={taskTitle}
              onChange={(e) => setTaskTitle(e.target.value)}
              className="w-full px-3.5 py-2 rounded-xl border border-zinc-700 bg-zinc-900 text-white placeholder-zinc-500 text-sm focus:outline-none focus:border-indigo-500"
            />
        ...
        <div className="space-y-2">
          {filteredTasks.length === 0 ? (
            <div className="p-8 text-center glass-card rounded-xl border border-zinc-800 text-xs text-zinc-500">
                ...
                >
                  <div className="flex items-center gap-3 min-w-0">
                    ...
                    <button
                      ...
                      title="Klik untuk mengubah status: To Do -> In Progress -> Done"
                    >
                      {t.status === 'done' ? (
                        ...
                        <Circle className="h-5 w-5 text-zinc-500 hover:text-zinc-300" />
                      )}
                    </button>
                    ...
                    <div className="min-w-0">
                      ...
                      <div className="flex flex-wrap items-center gap-2 mt-0.5 text-[11px]">
                        {/* Status Label */}
                        <span
                          className={`font-mono uppercase text-[10px] px-1.5 py-0.2 rounded font-medium ${
                            t.status === 'done'
                        ...
                        {/* Milestone info */}
                        {matchedMilestone && (
                          <span className="text-purple-400 font-mono text-[10px] flex items-center gap-0.5">
                            • <Flag className="h-2.5 w-2.5 inline" /> {matchedMilestone.title}
                          </span>
                        )}
