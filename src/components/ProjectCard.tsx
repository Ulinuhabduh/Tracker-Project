...
export function ProjectCard({
  ...
  return (
    <div className="glass-card rounded-2xl p-5 sm:p-6 relative flex flex-col justify-between transition-all duration-300 hover:shadow-xl hover:shadow-black/40 group border border-zinc-800/80 hover:border-zinc-700/80">
      {/* Top Header info */}
      <div>
        <div className="flex items-start justify-between gap-3 mb-3">
          <div className="flex flex-wrap items-center gap-2">
            <span className="text-[11px] font-mono font-medium px-2 py-0.5 rounded-md bg-zinc-800/80 text-zinc-300 border border-zinc-700/50">
              {project.category}
            </span>
            <span className={`text-[11px] font-medium px-2 py-0.5 rounded-md border ${priorityMeta.bg} ${priorityMeta.text} ${priorityMeta.border}`}>
              {priorityMeta.label}
            </span>
          </div>
        ...
        <h3 
          ...
          className="text-base sm:text-lg font-bold text-zinc-100 hover:text-indigo-400 cursor-pointer transition-colors leading-snug line-clamp-1"
        >
          {project.title}
        </h3>
        ...
        {project.tags && project.tags.length > 0 && (
          <div className="mt-3.5 flex flex-wrap gap-1.5">
            {project.tags.slice(0, 4).map((tag, idx) => (
              <span
                key={idx}
                className="inline-flex items-center gap-1 px-2 py-0.5 rounded text-[11px] font-mono text-zinc-400 bg-zinc-800/50 border border-zinc-800"
              >
            ...
            {project.tags.length > 4 && (
              <span className="text-[11px] font-mono text-zinc-500 self-center">
                +{project.tags.length - 4}
              </span>
            )}
          </div>
      ...
      <div className="mt-5 pt-4 border-t border-zinc-800/80">
        ...
        <div className="mt-4 flex items-center justify-between gap-2">
          ...
          <div className="flex items-center gap-2">
            <span className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-md text-[11px] font-medium border ${statusMeta.bg} ${statusMeta.text} ${statusMeta.border}`}>
          ...
          <button
            onClick={() => onOpen(project)}
            className="flex items-center gap-1 px-3 py-1.5 rounded-lg bg-zinc-800/80 hover:bg-zinc-750 text-xs font-semibold text-zinc-200 hover:text-white transition-all group-hover:bg-indigo-600/90 group-hover:text-white group-hover:shadow-md group-hover:shadow-indigo-600/20"
          >
            <span>Buka</span>
